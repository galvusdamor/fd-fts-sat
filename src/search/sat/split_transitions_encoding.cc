#include "split_transitions_encoding.h"
...
SplitTransitionsEncodingFactory::SplitTransitionsEncodingFactory(const options::Options &opts): SATEncodingFactory(opts.get<bool>("force_at_least_one_action")),
...
static shared_ptr<sat_search::SATEncodingFactory> _parse_split_transitions_sat_factory(options::OptionParser &parser) {
    ...
    if (parser.dry_run())
        ...
        return make_shared<SplitTransitionsEncodingFactory>(opts);
}

static options::PluginShared<sat_search::SATEncodingFactory> _plugin_split_transitions_sat_factory("split_transitions_sat", _parse_split_transitions_sat_factory);

void SplitTransitionsEncodingFactory::initialize() {
...
unique_ptr<sat_search::SATEncoding> SplitTransitionsEncodingFactory::createEncodingInstance(std::shared_ptr<sat_capsule> capsule){
	//bool oldStatisticsPrinted = statisticsPrinted;
	statisticsPrinted = true;
	return make_unique<SplitTransitionsEncoding>(capsule, fts, forceAtLeastOneAction, useSelfloopOptimisation, useLabelsInEffectsConstraints);
}
...
// Constructor
// ---------------------------------------------------------------------------
SplitTransitionsEncoding::SplitTransitionsEncoding(
...
set<int> SplitTransitionsEncoding::get_useful_diff_targets(int ts, int labelIndex, int target){
    set<int> targets;
    for (Transition transition : fts->get_ts(ts).get_transitions_with_label(getRelevantLabel(ts, labelIndex))){
        if(transition.src == target && transition.target != target){
...
void SplitTransitionsEncoding::appendNewTransitionVars(map<int, map<int, vector<pair<int, int>>>>& newSourceVars, 
                                                        map<int, map<int, vector<pair<int, int>>>>& newTargetVars){
    allTimesTransitionSourcesVars.push_back(newSourceVars);
    allTimesTransitionTargetsVars.push_back(newTargetVars);
}

void SplitTransitionsEncoding::generateTransitionVars(int /*fromTime*/) {
    map<int, map<int, vector<vector<int>>>> sourcesToTargets;
    map<int, map<int, vector<vector<int>>>> targetsToSources;

    map<int, map<int, vector<pair<int, int>>>> newSourceVars;
    map<int, map<int, vector<pair<int, int>>>> newTargetVars;
    ...
    for (int ts = 0; ts < fts->get_size(); ts++) {
        for (int label = 0; label < fts->get_num_labels(); label++) {
            ...
            sourcesToTargets[ts][label].resize(fts->get_ts(ts).get_size());
            targetsToSources[ts][label].resize(fts->get_ts(ts).get_size());
            for (size_t t = 0; t < transitions.size(); t++) {
                sourcesToTargets[ts][label][transitions[t].src].push_back(transitions[t].target);
                targetsToSources[ts][label][transitions[t].target].push_back(transitions[t].src);
            }
        }
    }
    ...
    for (int ts = 0; ts < fts->get_size(); ts++) {
        for (int label = 0; label < fts->get_num_labels(); label++) {
            if (useSelfloopOptimisation && isIrrelevantLabel(ts, label))
                continue;
            set<int> generatedSources;
            set<int> generatedTargets;
            vector<int> AMOSources;
            vector<int> AMOTargets;
            for(size_t src = 0 ; src < sourcesToTargets[ts][label].size() ; src ++){
                ...
                for(int target : sourcesToTargets[ts][label][src]){
                    if (useSelfloopOptimisation && (int)src == target && sourcesToTargets[ts][label][src].size() == 1) {
                        ...
                        if(generatedTargets.contains(target)){
                            for (auto& [generatedTarget, var] : newTargetVars[ts][label]){
                                if(generatedTarget == target)
                                    targetVar = var;
                            }
                        }else{
                            targetVar = sat->new_variable();
                            newTargetVars[ts][label].push_back({target, targetVar});
                            generatedTargets.insert(target);
                            AMOTargets.push_back(targetVar);
                            DEBUG(sat->registerVariable(
                                targetVar,
                                "TS:" + to_string(ts) + ";Label:" + to_string(label) +
                                " -- Target: " + to_string(target)));
                        }
                        targetDisjunction.push_back(targetVar);
                    }
                }
                ...
                if(sourcesToTargets[ts][label][src].size() > 0 && !generatedSources.contains(src)){
                    ...
                    sat->impliesOr(sourceVar, targetDisjunction);
                }
            }
            sat->atMostOne(AMOSources);
            sat->atMostOne(AMOTargets);
            
        }
    }
    appendNewTransitionVars(newSourceVars, newTargetVars);
}
...
void SplitTransitionsEncoding::appendPreconditions(vector<int>& impliesOrPrec, int ts, int relevantLabelPrec, int src){
    for (auto& [target, var] : allTimesTransitionTargetsVars.back()[ts][getRelevantLabel(ts, relevantLabelPrec)]){
        if (target == src && can_reach_via_not_selfloop(ts, relevantLabelPrec, src))//Need the function instead of just var!=-1 because of cases like s_2^{t+1}<- s_1^t -> s_1^{t+1} which should still not be counted for the s_1 target
            impliesOrPrec.push_back(var);
    }
}

void SplitTransitionsEncoding::appendEffects(vector<int>& impliesOrEff, int ts, int relevantLabelEff, int target, int time){
    set<int> usefulTargets = get_useful_diff_targets(ts, relevantLabelEff, target);
    for (auto& [targetVal, var] : allTimesTransitionTargetsVars.back()[ts][getRelevantLabel(ts, relevantLabelEff)]){
        if(!usefulTargets.contains(targetVal)){
            continue;
        }
...
vector<int> SplitTransitionsEncoding::get_all_sat_vars_moving_to_state(int ts, int label, int state, int fromTime){
    auto access = make_tuple(ts, label, state, fromTime);
    auto it = moving_to_state_vars.find(access);
    if (it == moving_to_state_vars.end()) {
        vector<int> ret;
        for (auto& [target, var] : allTimesTransitionTargetsVars.back()[ts][getRelevantLabel(ts, label)])
            if (target == state && var != -1){
                ret.push_back(var);
                break;
            }
...
vector<int> SplitTransitionsEncoding::get_all_sat_vars_moving_from_state(int ts, int label, int state, int fromTime){
    auto access = make_tuple(ts, label, state, fromTime);
    auto it = moving_from_state_vars.find(access);
    if (it == moving_from_state_vars.end()) {
        vector<int> ret;
        for (auto& [source, var] : allTimesTransitionSourcesVars.back()[ts][getRelevantLabel(ts, label)])
            if (source == state && var != -1){
                ret.push_back(var);
                break;
            }
...
vector<int> SplitTransitionsEncoding::get_all_non_encoded_self_loop_states(int ts, int label){
...
void SplitTransitionsEncoding::encode_label_consistency(int ts, int relevantLabelIndex, int time) {
    vector<int> labelSourceSATVars;
    vector<int> labelTargetSATVars;

    for (auto& [source, var] : allTimesTransitionSourcesVars.back()[ts][getRelevantLabel(ts, relevantLabelIndex)])
        if (var != -1)
            labelSourceSATVars.push_back(var);
    
    for (auto& [target, var] : allTimesTransitionTargetsVars.back()[ts][getRelevantLabel(ts, relevantLabelIndex)])
        if (var != -1)
            labelTargetSATVars.push_back(var);

    if (!labelSourceSATVars.empty()) {
        if ( (useSelfloopOptimisation && !containsSelfLoops(ts, getRelevantLabel(ts, relevantLabelIndex)))
                || !useSelfloopOptimisation)
            sat->impliesOr(getLabelSATVar(getRelevantLabel(ts, relevantLabelIndex), time), labelSourceSATVars);
        for (int v : labelSourceSATVars)
            sat->implies(v, getLabelSATVar(getRelevantLabel(ts, relevantLabelIndex), time));
    }

    if (!labelTargetSATVars.empty()) {
        if ( (useSelfloopOptimisation && !containsSelfLoops(ts, getRelevantLabel(ts, relevantLabelIndex)))
                || !useSelfloopOptimisation)
            sat->impliesOr(getLabelSATVar(getRelevantLabel(ts, relevantLabelIndex), time), labelTargetSATVars);
        for (int v : labelTargetSATVars)
            sat->implies(v, getLabelSATVar(getRelevantLabel(ts, relevantLabelIndex), time));
    }
}
...
void SplitTransitionsEncoding::encode_transition(const vector<vector<int>>& previousStateVars, const vector<vector<int>>& nextStateVars, int fromTime){
    for (int ts = 0; ts < fts->get_size(); ts++) {
...
void SplitTransitionsEncoding::encode_frame_axioms(const vector<vector<int>>& previousStateVars, const vector<vector<int>>& nextStateVars, int fromTime){
...
vector<vector<int>> SplitTransitionsEncoding::extractIntermediateStates(vector<int>& selectedLabels, vector<int>& currentLastState, vector<int>& /*nextState*/, int labelTimestep){
	...
	for(size_t l = 0 ; l < selectedLabels.size() - 1 ; l++){
		...
		for(int ts = 0 ; ts < fts->get_size() ; ts++){
			...
			for(auto& [target, var] : allTimesTransitionTargetsVars[labelTimestep-1][ts][selectedLabels[l]]){
                if(useSelfloopOptimisation && var == -1)
                    continue;
                if(ipasir_val(sat->solver, var) > 0){
                    intermediateState.push_back(target);
...
void SplitTransitionsEncoding::encode_r2_chains(int ts, int time){
    ...
    for(int states = 0 ; states < fts->get_ts(ts).get_size() ; states++){
        ...
        for (int relevantLabel = 0; relevantLabel < numRelevantLabels; relevantLabel++) {
            int index = 0;
            for (auto& [source, var] : allTimesTransitionSourcesVars.back()[ts][getRelevantLabel(ts, relevantLabel)]){
                if(useSelfloopOptimisation && var == -1)
                    continue;
            ...
            for (auto& [target, var] : allTimesTransitionTargetsVars.back()[ts][getRelevantLabel(ts, relevantLabel)]){
                if(useSelfloopOptimisation && var == -1)
                    continue;
                eventVars[relevantLabel].push_back(var);
    ...
    if(useSelfloopOptimisation){
        for (int relevantLabel = 1; relevantLabel < numRelevantLabels; relevantLabel++) {
            ...
            for (auto& [source, var] : allTimesTransitionSourcesVars.back()[ts][getRelevantLabel(ts, relevantLabel)]){
                if(var == -1){
                    auxVarsInPrec.push_back(auxVars[source][relevantLabel-1]);//Previous Aux Var
                }else{
