...
DATA_DIR = f"{os.path.dirname(os.path.abspath(__file__))}/data-icaps"
...
for directory in [d for d in Path(DATA_DIR).iterdir() if d.is_dir() and d != TARGET_DIR]:
        exp.add_fetcher(str(directory), merge=True, filter=[joint_domains, invert_min_negative_dominance, unsolvable_wo_mystery]) # filter=[ignore_unexplained_errors2, joint_domains, invert_min_negative_dominance, unsolvable_wo_mystery,rename_time_steps])
