...
REVISION = "1732de89794405e51ca13d36f578696bb96de2b6"
REVISIONS = [REVISION]

CONFIGS = []

encodings = {
		"chains_slf_lg_rcpo": "label_sat(encoding=CHAINS_PARALLEL,use_self_loop_optimisation=true,use_label_group=true,use_empty_pillars=true,use_empty_rows=true,use_empty_cols=true,use_positive_one=true,force_at_least_one_action=false)",
...
searches = {
        ("CM2-" + name) : "rintanen(encoder=" + encodings[name] + ", solver_quiet=true,length_strategy=by_iteration(),memory_limit_mb=3500,max_parallel_calls=20,scheduler_interval=1,schedule_formula_as_one=true)" for name in encodings
}
...
exp.add_report(AbsoluteReport(attributes=attributes, filter_algorithm=["C-chains_slf_lg_rcpo-shr","CM2-chains_slf_lg_rcpo-shr"], filter=[filters.filter_bdd_known_unexplained_errors,filters.filter_kissat_known_unexplained_errors]), outfile=f"{SCRIPT_NAME}-memory-variants.html")
