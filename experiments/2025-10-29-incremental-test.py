...
REVISION = "d14c3771cd781c14ac48b0d5e3bc325f38d09eb1"
REVISIONS = [REVISION]
...
searches = { ("IIc-" + name): ("incremental_sat(encoder=" + encodings[name].replace("FORCE","true") +", solver_quiet=true,strategy=INIT,length_strategy=one_by_one())") for name in encodings} | {
         ("IGc-" + name): ("incremental_sat(encoder=" + encodings[name].replace("FORCE","true") +", solver_quiet=true,strategy=GOAL,length_strategy=one_by_one())") for name in encodings}  | {
         ("IMc-" + name): ("incremental_sat(encoder=" + encodings[name].replace("FORCE","true") +", solver_quiet=true,strategy=MIDDLE,length_strategy=one_by_one())") for name in encodings} | {
         ("A1c-" + name): ("sat(encoder=" + encodings[name].replace("FORCE","true") +", solver_quiet=true,continue_after_first_plan=false,length_strategy=one_by_one())") for name in encodings}
        #| {("A-it" + name): ("sat(encoder=" + encodings[name].replace("FORCE","false") + ", solver_quiet=true,continue_after_first_plan=false,length_strategy=by_iteration())") for name in encodings}
        #| {("CMit" + name) : ("rintanen(encoder=" + encodings[name].replace("FORCE","false") + ", solver_quiet=true,length_strategy=by_iteration(),memory_limit_mb=3500,max_parallel_calls=20,scheduler_interval=1,schedule_formula_as_one=true)") for name in encodings}
...
for s_name, s_opt in searches.items():
    for t_name, t_opt in TRANSFORM_OPTS.items():
        if s_name == "ff-pure":
            CONFIGS.append(IssueConfig(f'{s_name}{t_name}', ['--search',  f'{s_opt}'], driver_options=DRIVER_OPTS, build_options=["-j16", "-s/gpfs/home2/behnkeg/software/cadical/build", "--ipasir","-lcadical"]))
        else:
            CONFIGS.append(IssueConfig(f'{s_name}{t_name}', t_opt + ['--search',  f'{s_opt}'], driver_options=DRIVER_OPTS, build_options=["-j16", "-s/gpfs/home2/behnkeg/software/cadical/build", "--ipasir","-lcadical"]))
